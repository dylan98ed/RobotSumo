...
{ 
  ...
  if(medirDistancia(trigger, echo) <= 50) // el ring tiene un area de 175cm o 154cm -- alrededor hay 100cm vacio
  {
    maquinaEstados('m');
  }
  else
  {
    if((checkSenLineaTodos() == LINEA_DETECTADA) && (medirDistancia(trigger, echo) <= 50))
    {
      maquinaEstados('s');
...
void tiempoEsperaConInterrup(int segundos, int trigger, int echo)
{
  ...
  { 
    ...
    if(medirDistancia(trigger, echo) <= 50)
    {
      maquinaEstados('m');
      break;
    }
